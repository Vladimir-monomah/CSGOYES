...
/*
=====================================================
 CSGOYES - by SoftNews Media Group 
=====================================================
...
if ($is_loged_in) {
	...
	foreach ( $options as $sub_options ) {
		...
		$menu_item[] = "<li class=\"dark-nav{$active_menu}\">
		<span class=\"glow\"></span><a class=\"accordion-toggle\" data-toggle=\"collapse\" 
		href=\"#submenu{$subs}\"><i class=\"icon-{$icon} icon-2x\"></i><span>{$menu_item_header} 
		<i class=\"icon-caret-down\"></i></span></a><ul id=\"submenu{$subs}\" class=\"collapse{$collapsed}\">"
		.$submenu_item."</ul></li>";	
	}
	
	if( count( $menu_item ) ) $sidebar= "<ul class=\"nav navbar-collapse collapse navbar-collapse-primary\">
	<li><span class=\"glow\"></span><a href=\"?mod=options&action=options\"><i class=\"icon-globe icon-2x\">
	</i><span>{$lang['header_all']}</span></a></li>".implode("", $menu_item)."</ul>";
	else $sidebar= "<ul class=\"nav navbar-collapse collapse navbar-collapse-primary\">
	<li><span class=\"glow\"></span><a href=\"?mod=options&action=options\"><i class=\"icon-globe icon-2x\"></i>
	<span>{$lang['header_all']}</span></a></li></ul>";
...
<head>
  ...
  <title>CSGOYES - {$lang['skin_title']}</title>
...
"{$lang['media_upload']}"];
var cal_language   = {en:{months:['{$lang['January']}','{$lang['February']}','{$lang['March']}','{$lang['April']}',
'{$lang['May']}','{$lang['June']}','{$lang['July']}','{$lang['August']}','{$lang['September']}','{$lang['October']}',
'{$lang['November']}','{$lang['December']}'],dayOfWeek:["{$langdate['Sun']}", "{$langdate['Mon']}", "{$langdate['Tue']}", 
"{$langdate['Wed']}", "{$langdate['Thu']}", "{$langdate['Fri']}", "{$langdate['Sat']}"]}};
//-->
</script>
...
<nav class="navbar navbar-default navbar-inverse navbar-static-top" role="navigation">
  <div class="navbar-header">
    <a class="navbar-brand" href="?mod=main">CSGOYES</a>
...
<!--MAIN area-->
	...
	<div class="login box" style="margin-top: 100px;">
      ...
      <div class="box-header">
        <span class="title">{$lang['skin_title']} CSGOYES</span>
      </div>
	  
      <div class="box-content padded">
        <form  name="login" action="" method="post" class="separate-sections">
		  ...
		  <div class="input-group addon-left">
			<input type="checkbox" class="icheck" name="login_not_save" id="login_not_save"
			 value="1"/><label for="login_not_save">&nbsp;{$lang['m_not_save']}</label>
			<button type="submit" class="btn btn-blue btn-block">{$lang['login_button']} 
				<i class="icon-signin"></i></button>
          </div>
